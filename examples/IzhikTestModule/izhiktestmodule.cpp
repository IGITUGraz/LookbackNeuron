...
mynest::IzhikTestModule izhiktestmodule_LTX_mod;

// -- DynModule functions ------------------------------------------------------

mynest::IzhikTestModule::IzhikTestModule()
{
#ifdef LINKED_MODULE
...
const std::string mynest::IzhikTestModule::name(void) const
{
  return std::string("IGI SWTA Module"); // Return name of the module
}

const std::string mynest::IzhikTestModule::commandstring(void) const
{
  // Instruct the interpreter to load izhiktestmodule-init.sli
  return std::string("(izhiktestmodule-init) run");
}

//-------------------------------------------------------------------------------------

typedef nest::STDPConnection<nest::TargetIdentifierPtrRport> IncomingSynapseType;

void mynest::IzhikTestModule::init( SLIInterpreter* i )
{
  ...
  */
  nest::kernel().model_manager.register_node_model<mynest::izhikevich_prenorm>("pp_izhikevich_prenorm");
  ...
  */
  ...
  nest::kernel().model_manager.register_connection_model<mynest::ConnectionT, nest::LookBackConnectorModel>("stdp_connection_norm");

}  // IzhikTestModule::init()
